...
To finish missing target-ratio covers in Codex App, read `workflow/context-image2-cover-requests.json`, invoke the installed `imagegen` Skill / Context Image2 built-in `image_gen` for the missing native-ratio bitmap, inspect it, then run `scripts/ingest-codex-image2-cover-target.mjs --topic <topic-dir> --target <target-id> --source <codex-imagegen-png> --inspection-status passed-human-or-vision-review`. This ingest step consumes the existing `workflow/cover-image2-prompts.json` evidence, verifies the generated bitmap has the target ratio, writes exact platform PNG/JPG exports, records the canonical `completed` Image2 request, moves each target from `pendingNativeTargetRatioPrompts` to `fulfilledNativeTargetRatioExports`, and only then marks `uploadReady: true`. The explicit API alternative remains `scripts/generate-cover-targets-image2.mjs --root <batch-root-or-topic-root>` after `OPENAI_API_KEY` is available. If neither route has a real inspected Image 2/Codex bitmap, the target must stay pending rather than create local substitute artwork.

The primary cover readiness gate applies only to the standalone platform submission thumbnail uploaded after the MP4 is complete. `video-opening`, the first rendered frame, `native-final-cover-*` review images, and files under `评审级封面-非上传终版/` never satisfy it. The canonical request must be marked `completed`, record `purpose: "platform-submission-cover"`, `provider: "codex-context-image2"`, `tool: "image_gen"`, a real `actualOutput`, and passed human/vision inspection. Ingest with `--inspection-status passed-human-or-vision-review`, then run `scripts/validate-platform-submission-cover.mjs --out <topic-dir>`. Use `--require-all-platform-covers` only when claiming every requested platform ratio is complete.

The final-render entrypoint does not enforce the platform-cover gate. The video-production lane may compose the MP4 while the platform-cover agent is still generating or awaiting inspection. The independent publishing-cover validator must fail the platform-cover claim when the primary request is pending or the native bitmap is missing, and must surface the Image2 Skill invocation and ingest command instead of silently accepting a local SVG/review cover.
...
4. Ingest every Context Image2 PNG through `scripts/ingest-codex-image2-cover-target.mjs` so the package can verify ratio, update `workflow/cover-size-selection.json`, and move prompts from pending to fulfilled.
