...
- By default, the request list contains every target actually planned in `workflow/cover-image2-prompts.json`; planning nine platform-native variants and then silently generating only one is forbidden. `coverTargetIds` / `coverTargets` may select an explicit subset. A deliberate primary-only run must opt into `coverPrimaryOnly:true`, `coverAllPlatforms:false`, or `generateAllPlatformCovers:false`. The request manifest must prove expected count equals actual count, and bounded concurrency controls simultaneous generation only—not the total request count.
...
- In Codex App, prepare every pending request through the standalone cover Skill, dispatch the complete set with the built-in `image_gen` tool, bind each output to its target, and converge through one locked batch ingest. The default worker-pool width is the pending count capped at 9; a lower explicit value limits simultaneous provider calls only and must never reduce the request-list count. When an explicit API-key route is authorized, `scripts/generate-cover-targets-image2.mjs --root <topic-or-batch-root> --concurrency 9` may complete the same request set. `--limit`, array slicing, and manual batches are forbidden.
...
Use Context Image2 / Codex built-in `image_gen` for the complete cover in Codex App sessions: depth, lighting, material, target/check/proof objects, premium finish, and the main Chinese title/subtitle/badge typography. The core workflow still owns the strategy, prompt text, target dimensions, whitelist, and QC through `workflow/cover-design.json`, `workflow/cover-image2-prompts.json`, `workflow/cover-size-selection.json`, and `workflow/context-image2-cover-requests.json`. Keep deterministic overlay layers only for fallback/text-repair after review, not as the default typography system.
...
The deliverable must never be cover-less. When a run has no native upload-ready target-ratio Image 2 cover at all — the common default口播/`image2-dryrun` case — the workflow may surface review-grade draft covers inside `最终成品/` under a clearly-labeled `评审级封面-非上传终版/` subfolder only for same-canvas review surfaces such as the 16:9 master/video-opening family. This subfolder is explicitly not the upload selection: the aspect-named upload group folders (`横版16比9/` etc.) still contain native upload-ready target-ratio covers only, and `humanSelectionContainsOnlyUploadReady`/`nonUploadReadyVisualFilesCopied` continue to describe those upload group folders. For missing non-16:9 target-ratio covers — including `4:3`, `3:4`, square, Reels, and Bilibili common — do not write target-size review-grade drafts, because they are easily mistaken for final covers and often expose truncation/recomposition artifacts. Those targets must stay in `需原生重生成清单.md` and `workflow/context-image2-cover-requests.json` until Context Image2 / Codex built-in `image_gen` or the explicit Image2 API generates native target-ratio bitmaps. Review-grade drafts keep `uploadReady: false`, are recorded under `reviewGradeCoverFiles` / `reviewGradeCoverDirectory` in `workflow/cover-size-selection.json`, and every affected size still appears in `需原生重生成清单.md`;正式上传前仍需按该尺寸重新生成原生 Image 2 封面. This review-grade draft is a different category from the `COVER_LOCAL_RECOMPOSITION_PREVIEW=1` `localTargetRatioRecomposition` preview, which remains under the separate top-level `封面预览-非上传终版/` directory.
...
If a native Image 2/Codex target-ratio asset is not yet present and the user explicitly asks to preview missing `4:3` or `3:4` gaps, the workflow may output a clearly marked `review-only-local-target-ratio-recomposition` temporary file under `封面预览-非上传终版/` only when `COVER_LOCAL_RECOMPOSITION_PREVIEW=1` is set. This is a native-canvas local recomposition, not a true Image 2 native bitmap: it must rebuild the hierarchy for the target canvas, avoid blank areas, avoid overlapped/duplicated artwork, avoid hard crop, keep `uploadReady: false`, keep metadata fields `image2NativeTargetRatioReady: false` and `localTargetRatioRecomposition: true`, and remain excluded from `最终成品/` so later Codex built-in Image 2 or explicit API regeneration can replace it cleanly. Without that explicit preview switch, missing `4:3` / `3:4` / square / Reels / Bilibili common targets must produce no target-size local image at all; they stay prompt-pending for Image2.

To finish missing target-ratio covers in Codex App, run the standalone prepare command, dispatch every job in `workflow/cover-image2-dispatch-plan.json` through the installed `imagegen` Skill / Context Image2 built-in `image_gen`, and record each target-bound result. Then write two auditable files per target: a generation receipt containing the canonical request id, prompt hash, output hash, source path, `provider: codex-context-image2`, `tool: image_gen`, and generation timestamp; and an independent human/vision inspection record containing the same target/output hash, passed status, inspector type, and inspection timestamp. Run the standalone locked batch-ingest coordinator only after every target is generated and inspected. It rejects arbitrary local/review PNGs, verifies ratios and hashes, writes exact platform exports, and runs cover-only QC once. It does not run full-video QC. `--inspection-status` alone can never mark a cover upload-ready.

The primary cover readiness gate applies only to the standalone platform submission thumbnail uploaded after the MP4 is complete. `video-opening`, first frames, native-page review images, and review-only cover directories never satisfy it. The canonical request must retain the verified generation receipt and inspection record, real output, provider/tool, and upload-ready native ratio. Full QC is authoritative; ingest metadata alone cannot set package pass.

MP4 composition may finish while the platform cover is pending, but terminal publishing promotion always enforces the platform-cover gate. A pending cover retains `renders/final.mp4` only as a review video; it cannot create final delivery copies or a passing package. Valid ingest automatically reruns full QC and promotes only when every publishing check passes.

## Parallel Cover Lane

The cover package is independent of final video composition. Once the title, narration/script, frames, and design plan are locked, the workflow should start `writeCoverArtifacts()` as the cover-design lane and write `workflow/cover-parallel-execution.json`. TTS, subtitle timing, native-page assembly, and MP4 rendering may proceed or fail separately. If a personal-IP/native-final run blocks because verified native page provenance is missing, the block must happen after the core cover lane has completed or failed; the package should still contain the core cover contract artifacts instead of only a native-page review cover.

## Context Image2 Handoff Contract

The cover handoff is mandatory because Context Image2 is an interactive Codex App capability while the core cover logic is deterministic workflow code.

1. Run the core cover engine first. It writes the click strategy, platform logic, prompt text, target sizes, and pending/upload-ready state.
2. Use `workflow/context-image2-cover-requests.json` as the only source of truth for Context Image2 generation. Each request must point to a prompt under `prompts/context-image2-covers/` and preserve the target id, dimensions, ratio, and ingest command.
3. Generate only missing or non-upload-ready targets. Do not regenerate already upload-ready native Image2 targets during completion.
4. Record every target-bound output, then run `skills/codex-video-cover-generation/scripts/ingest-codex-image2-cover-batch.mjs` once so the package can preflight all ratios/evidence, serialize shared-state updates, update `workflow/cover-size-selection.json`, and move prompts from pending to fulfilled without races.
5. If `workflow/context-image2-cover-requests.json` is missing, empty while targets are pending, or bypassed with ad hoc prompt text, the cover package is incomplete even if local review PNGs exist.
6. When the installed `imagegen` / Context Image2 capability is available in the active Codex task, a requested final video task must continue through generation, inspection, ingestion, and `validate-platform-submission-cover.mjs`; do not stop at the pending handoff artifact or describe a review-only SVG/PNG as the corresponding final cover.
...
- A pending primary platform cover being packaged under final/delivery names. Pending-cover output is `video-review-ready` only; it must use `review-manifest.json`, keep the review MP4 under `renders/`, and must not create root `final.mp4`, a title-named final copy, `delivery-manifest.json`, or `delivery.html`.
