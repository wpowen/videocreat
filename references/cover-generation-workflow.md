...
5. Record each generated result through `record-cover-image2-dispatch-result.mjs`. After visual review passes, use `scripts/record-cover-generation-evidence.mjs` to write the request-bound generation receipt and independent human/vision inspection record. Never hand-author evidence JSON.
...
- Evidence recording accepts only an output beneath `${CODEX_HOME:-$HOME/.codex}/generated_images` (or the explicit test override), validates native ratio and prompt parity, and writes to the request-declared evidence paths.
