...
When the user asks for a个人 IP 图解图片、竖屏知识卡样张、横屏图解页、手机海报式个人 IP 图解、or asks to preview the IP page before video assembly, use the standalone source-page route. For final-quality Codex App output, this route uses Codex Context Image2 / built-in `image_gen` from `workflow/context-image2-persona-page-requests.json`; GPT Image 2 through the API is an explicit non-default automation path, and prompt-only handoff is incomplete until Codex Image2 pages are generated and ingested. Default aspect is vertical `9:16`; pass `--aspect 16:9` for horizontal images.
...
- Explicit IP-diagram fields: `ipDiagram`, `ipDiagramCreator`, `ipDiagramCreatorPrimary`, `primaryIpDiagramCreator`, `plannerDriver: "ip-diagram-creator"`, `primaryVisualSystem: "ip-diagram-creator"`, or `visualMode: "ip-diagram"`, normalized as real toggles rather than JavaScript truthiness.
- Explicit personal-IP fields: `personalIp`, `creatorPersona`, `ipCharacter`, `personalIpAssets`, `authorizedRoleAssets`, `personalIpReference`, `personalIpPhoto`, or `personalIpImage`. Boolean/string/object values must be normalized: `true`/`"true"`/`"on"` and identity-bearing objects activate; `false`/`"false"`/`"off"`/`"no"`/`0` and `{ "enabled": false }` do not.
- Strong non-negated text signals: `个人IP`, `IP角色`, `图解角色`, `角色主锚图`, `三张角色`, `creator persona`, `personal brand`, `personal IP`, or `diagram persona`. Negated language such as `不要个人 IP`, `非个人 IP`, `do not use personal IP`, or `without creator persona` must be stripped before positive matching.
- Generic teaching, knowledge-card, tutorial, course, PPT, explainer, science, checklist, relationship, atlas, hook, mood, UI, or recap wording does not activate personal IP. Route those content shapes through `workflow/visual-series-routing-plan.json` unless an explicit IP-diagram field or non-negated personal-IP signal is also present.
- `执行 Agent` / `Agent 协作` and director/role-anchor wording may activate this route only when they explicitly request the IP-diagram capability, not merely because a general workflow happens to contain agents.
...
Primary planner route is active when the brief explicitly requests `ip-diagram-creator` as a planner/visual system or contains a strong non-negated personal-IP signal. Natural-language requests such as `生成个人 IP 手绘图解视频` must be treated as a primary IP diagram route, even if the user did not also say `课程` or `教学`. If both personal IP and another `visualSeriesId` are requested, personal IP wins and the conflict must be recorded in `workflow/visual-series-routing-plan.json`; the series route may not overwrite the persona route. A personal-IP signal must also activate native source jobs, fixed/default persona rendering, and mandatory `native-final-video`; missing native page provenance is a blocker, not permission to fall back to integrated HTML composition.
