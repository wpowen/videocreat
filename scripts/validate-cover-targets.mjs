...
function main() {
  ...
  expect(/function updateContextRequests/.test(codexImage2IngestScript) && /request\.purpose = "platform-submission-cover"/.test(codexImage2IngestScript) && /inspectionPassed/.test(codexImage2IngestScript), "Codex Image2 ingest must complete canonical platform-submission-cover requests and require visual inspection", failures);
  expect(/primaryPlatformUploadCoverReady/.test(codexImage2IngestScript) && /allRequestedPlatformUploadCoversReady/.test(codexImage2IngestScript) && /qc\.finalCoverQualityEligible = allReady/.test(codexImage2IngestScript), "Codex Image2 ingest must keep primary platform readiness separate from all-target package completion", failures);
  expect(/finalDeliveryDirectory/.test(codexImage2IngestScript) && /copyFileSync\(join\(topicDir, pngFile\)/.test(codexImage2IngestScript), "Codex Image2 ingest must copy the standalone submission cover into the final delivery directory", failures);
  ...
  expect(/stage:\s*"pre-cover-full-render"[\s\S]*writePersonalIpNativeFinalBlockedManifest/.test(script), "personal-IP pre-cover native-final blocker must preserve the independent cover lane contract", failures);
  expect(/stage:\s*"pre-audio-full-render"[\s\S]*writePersonalIpNativeFinalBlockedManifest/.test(script), "personal-IP pre-audio native-final blocker must preserve the independent cover lane contract", failures);
